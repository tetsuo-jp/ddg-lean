...
MUTATIONS = [
    ...
    ("singleEdge ranks the child below the parent",
     "  rk v := if v = a then 1 else 0",
     "  rk v := if v = a then 0 else 1"),
    ("the empty forest is given a parent",
     "def empty (V : Type) : RootedForest V where\n  parent _ := none",
     "def empty (V : Type) : RootedForest V where\n  parent v := some v"),
    ("one-edge corollary drops the distinctness hypothesis",
     "(B : Bary P N) {a b : V} (hab : a \u2260 b) (hb : b \u2208 N a) (ha : a \u2208 N b) :",
     "(B : Bary P N) {a b : V} (hab : a = a) (hb : b \u2208 N a) (ha : a \u2208 N b) :"),
